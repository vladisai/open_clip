        ...
        if self.world_size > 1:
            ...
            if self.local_loss:
                ...
                similarities = torch.einsum(
                    "nc,mc->nm", text_features, all_text_features
                )
            else:
                logits_per_image = (
                    logit_scale * all_image_features @ all_text_features.T
                ...
                logits_per_text = logits_per_image.T
                similarities = torch.einsum(
                    "nc,mc->nm", all_text_features, all_text_features
                )
        else:
            logits_per_image = logit_scale * image_features @ text_features.T
            logits_per_text = logit_scale * text_features @ image_features.T
            similarities = torch.einsum("nc,mc->nm", text_features, text_features)

        return logits_per_image, logits_per_text, similarities

    def forward(self, image_features, text_features, logit_scale, output_dict=False):
        device = image_features.device
        logits_per_image, logits_per_text, similarities = self.get_logits(
            image_features, text_features, logit_scale
        )

        # normalize similarities by the maximum similarity
        similarities_normed = (
            similarities / similarities.max(dim=1, keepdim=True).values
        )

        labels = self.get_ground_truth(device, logits_per_image.shape[0])
        # convert labels to one-hot
        labels_oh = F.one_hot(labels, num_classes=logits_per_image.shape[1])

        combined_weights = (
            self.coeff * labels_oh + (1 - self.coeff) * similarities_normed
        )
        p = torch.nn.functional.softmax(
            combined_weights / self.weights_temperature, dim=1
        )

        loss = -torch.sum(
            p * nn.functional.log_softmax(logits_per_image, dim=1), dim=1
        ).mean(dim=0)
